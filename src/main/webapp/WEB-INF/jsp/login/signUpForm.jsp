...
<script type="text/javascript">
	$(function() {
		...
		// 비밀번호 유효성검사
		$("div#user_password input[type=password]#user_password")
				.focusout(
						function() {

							var password = $(
									"div#user_password input[type=password]")
									.val();
							...
							if (checkNumber < 0 || checkEnglish < 0) {
								...
								$("label#password_check")
										.html(
												"&nbsp; X 숫자, 영문자 혼용해 다시 작성해주세요.  TT _ TT");
								$("div#user_password input[type=password]")
										.focus();
								return false;
							}
							if (/(\w)\1\1\1/.test(password)) {
								...
								$("label#password_check")
										.html(
												"&nbsp; X 같은 문자를 4번이상 사용하실 수 없습니다.  TT _ TT");
								$("div#user_password input[type=password]")
										.focus();
								return false;
							}
							...
							if (!/^(?=.*[a-zA-Z])(?=.*[!@#$%^*+=-])(?=.*[0-9]).{8,25}$/
								...
								$("label#password_check")
										.html(
												"&nbsp; X 숫자+영문자+특수문자 조합으로 8자리 이상 사용해 주세요.  TT _ TT");
								$("div#user_password input[type=password]")
										.focus();
								return false;
							}
							...
							$("label#password_check").html(
									"&nbsp; O 유효한 비밀번호 입니다. ^ __ ^");
							return true;
						});

		//비밀번호 재확인
		$("div#user_re_password input[type=password]#user_re_password")
				.focusout(
						function() {
							var password = $(
									"div#user_password input[type=password]")
							...
							var re_password = $(
									"div#user_re_password input[type=password]")
									.val();

							if (!(password == re_password)) {
								$("label#re_password_check")
										.css("color", "red");
								$("label#re_password_check").html(
										"&nbsp; X 비밀번호가 일치하지 않습니다.  TT _ TT");
								$("div#user_re_password input[type=password]")
										.focus();

								return false;
							} else {
								$("label#re_password_check").css("color",
										"green");
								$("label#re_password_check").html(
										"&nbsp; O 비밀번호가 일치합니다.  ^ __ ^");
			...
			<div class="panel panel-default">
				<div class="panel-heading">
					<div id="top_title1">
						...
						<div class="top_title2">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;기본 정보를
							정확히 입력해주시기 바랍니다.</div>
					</div>
				</div>
				<form action="/user/signUp.do" method="post" id="signup_form">
					<div id="step02_total">
						...
						<div id="user_re_password">
							...
							<div class="right_col">
								...
								<br> <label id="re_password_check"></label>
							</div>
						</div>

						<div id="signUp_btn">
							<div id="align_div">
								<div id="okBtn" class="btn btn-default">회원가입</div>

								<div id="resetBtn" class="btn btn-default">새로 입력</div>
